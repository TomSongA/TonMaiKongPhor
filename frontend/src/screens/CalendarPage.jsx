...
import { filterSamplesByTimeRange } from '../lib/sensorLogic'
import { buildCalendarMonth, fetchReadingsRange } from '../lib/sensorApi'
import './Page.css'
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        const rows = await fetchReadingsRange(from, to)
        if (cancelled) return
        setMonthData(buildCalendarMonth(y, m, rows))
      } catch {
        if (cancelled) return
        setMonthData(buildCalendarMonth(y, m, []))
      }
    }
    run()
    return () => {
  ...
  const avgStress =
    selected?.samples?.length > 0
      ? selected.samples.reduce((a, b) => a + b.psiScore, 0) / selected.samples.length
      : null
    ...
    <div className="page">
      <header className="page-head">
        ...
        <p className="page-desc">
          Each day shows its wellness score (higher means happier plants). A dash means we did not receive readings.
        </p>
      </header>
            ...
            {days.map((d) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="cal-day-psi" title="Wellness (100 − avg stress)">
                    {hasData ? d.psi : '—'}
                  </span>
                </button>
              )
              ...
              {avgStress != null ? (
                <>
                  <p className="detail-summary">
                    Daily avg wellness: <strong>{Math.round(100 - avgStress)}</strong>
                    {selected.stressed ? (
                      <span className="badge badge--warn">Elevated stress (avg)</span>
                    ) : (
                      <span className="badge badge--ok">Mostly healthy (avg)</span>
                    )}
                  </p>
                  ...
                  <div className="chart-wrap chart-wrap--tall">
                    {chartRows.length > 0 ? (
                      ...
                      <p className="page-foot">No samples in the selected time range.</p>
                    )}
                  </div>
                </>
              ) : (
                <p className="page-foot">No readings for this day.</p>
              )}
            </section>
          ) : (
