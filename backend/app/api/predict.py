...
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise HTTPException(status_code=503, detail="ML model not trained yet. Run python -m app.ml.train first.")
    if not os.path.exists(CLASSIFIER_PATH):
        raise HTTPException(status_code=503, detail="ML classifier not trained yet. Run python -m app.ml.train first.")
    if not os.path.exists(SCALER_PATH):
        raise HTTPException(status_code=503, detail="Scaler not found. Run python -m app.ml.train first.")
...
def get_prediction(hours_ahead: int = 3, db: Session = Depends(get_db)):
    ...
    if hours_ahead < 1 or hours_ahead > 12:
        raise HTTPException(status_code=400, detail="hours_ahead must be between 1 and 12.")
    ...
    if len(readings) < 2:
        raise HTTPException(status_code=404, detail="Not enough data to make a prediction. Need at least 2 readings.")
...
def get_trend(db: Session = Depends(get_db)):
    ...
    if slope > 2:
        ...
        trend, icon = "falling", "⬇️"
    else:
        trend, icon = "stable", "➡️"

    return {"trend": trend, "icon": icon, "slope": round(slope, 3)}
